...
struct request *new= malloc(sizeof(struct request));//create new cus for the user input
 ...
 for(temp=list;temp!=NULL;temp=temp->next){
	 if(new->room_number==temp->room_number){//if the room exists
 ...
 if(list->next==NULL){ //if there 2 elements 
     if(strcmp(new->last,list->last)>0){ //compare  last name
          list->next=new;
          new->next=NULL;
          return list;
      }
      ...
      if(strcmp(new->last,list->last)==0){  //if there last names the same
          if(strcmp(new->first,list->first)<=0){//compare  first name
              new->next=list;
              list=new;
              return list;
          }
 ...
 for(cur=list->next, pre= list;cur!=NULL;pre=cur, cur=cur->next){
     if(strcmp(new->last,pre->last)<0){ //compare last name
         new->next=pre;
         list=new;
         return list;
     }
     if(strcmp(new->last,pre->last)>0&&strcmp(new->last,cur->last)<0){ //compare their last name with the current and previous element
         new->next=cur;
         pre->next=new;
         return list;
     }
...
struct request *delete_from_list(struct request *list)
{
	...
	{  //while the list is not pointing to NULL
      ...
      //get info from user input
		...
		{
            if(cur->room_number==user->room_number&& strcmp(user->first,cur->first)==0&& strcmp(user->last,cur->last)==0){  //if the information matches
                if(cur==list){  //if the request in the first element 
                    list=list->next;
